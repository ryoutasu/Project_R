...
function mt:set_point(point, isSmooth)
    if self:is_destroyed() then
        return
    end
    self._base:api_force_transmit_new(point._base, isSmooth or false)
end

function mt:can_collide_with_point(point, radius)
    return GameAPI.unit_can_collide_with_point(self._base, point._base, radius)
end
